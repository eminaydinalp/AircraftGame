...
public class AeroplaneController : MonoBehaviour
{
    ...
    [SerializeField] private Slider _maxEngine;
    ...
    {
        EventManager.OnTriggerLastCheckPoint += CloseSlider;
    }
    ...
    {
        _maxEngine.gameObject.SetActive(false);
    }

    private void Update()
    {
        if (m_Rigidbody.velocity.z > _maxEngine.value && _landingGear.m_State == LandingGear.GearState.Raised)
        {
            m_Rigidbody.velocity = new Vector3(m_Rigidbody.velocity.x, m_Rigidbody.velocity.y, _maxEngine.value);
        }

        _rotator.speed = isLanding ? Mathf.Lerp(_rotator.speed, 0f, Time.deltaTime * 0.15f) : Mathf.Lerp(_rotator.speed, _maxEngine.value * 20f, Time.deltaTime * 0.3f);
        /*if(_landingGear.m_State == LandingGear.GearState.Lowered) return;
        m_Rigidbody.velocity = new Vector3(m_Rigidbody.velocity.x, m_Rigidbody.velocity.y, Mathf.Clamp(m_Rigidbody.velocity.z, 0,_maxEngine.value));*/
        //m_Lift = _landingGear.m_State == LandingGear.GearState.Lowered ? 0.1f : 0.002f;

        /*if (_landingGear.m_State == LandingGear.GearState.Lowered)
        {
    ...
    public void Takeoff()
    {
        ...
        {
            m_Rigidbody.velocity = (Vector3.forward * (_maxEngine.value * 10 * Time.deltaTime));
        }

        if (transform.position.z > 40f && transform.position.z < 50f)
        {
    ...
    private void ControlThrottle()
    {
        ...
        EnginePower = Throttle * _maxEngine.value;
    }
