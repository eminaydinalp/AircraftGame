...
public class AeroplaneController : MonoBehaviour
{
    ...
    {
        maxEngine.gameObject.SetActive(false);
    }

    private void Update()
    {
        _rotator.speed = isLanding ? Mathf.Lerp(_rotator.speed, 0f, Time.deltaTime * 0.15f) : Mathf.Lerp(_rotator.speed, maxEngine.value * 20f, Time.deltaTime * 0.15f);
        
        if (m_Rigidbody.velocity.z > maxEngine.value && _landingGear.m_State == LandingGear.GearState.Raised)
        {
            m_Rigidbody.velocity = new Vector3(m_Rigidbody.velocity.x, m_Rigidbody.velocity.y, maxEngine.value);
        }
    ...
    public void Takeoff()
    {
        ...
        {
            m_Rigidbody.velocity = (Vector3.forward * (maxEngine.value * 10 * Time.deltaTime));
        }

        if (transform.position.z > 120f && transform.position.z < 150f)
        {
    ...
    {
        if (maxEngine.value <= 0 || transform.position.z < 150f) return;
    ...
    private void ControlThrottle()
    {
        ...
        EnginePower = Throttle * maxEngine.value;
    }
